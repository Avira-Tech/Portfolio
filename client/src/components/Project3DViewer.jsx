...
const Scene = ({ currentSection, setActiveSection }) => {
  ...
  useFrame((state) => {
    ...
    const effectiveOffset =
      currentSection !== null ? currentSection / 6 : offset; // 6 intervals for 7 sections
    ...
    const colorIndex = Math.min(
      Math.floor(effectiveOffset * colors.length),
      colors.length - 1,
    );
  ...
  const projectInfo = [
    {
      ...
      desc: "Revolutionizing digital learning with interactive 3D modules and real-time collaboration.",
    },
    {
      title: "RFID Security",
      category: "IOT",
      color: "from-emerald-400 to-teal-600",
      desc: "Enterprise-grade access control systems integrated with smart cloud infrastructure.",
    },
    {
      title: "Stellar Campus",
      category: "CLOUD",
      color: "from-orange-400 to-red-600",
      desc: "A comprehensive management ecosystem for modern educational institutions.",
    },
    {
      title: "Krushi Mitra",
      category: "AGRI",
      color: "from-lime-400 to-green-600",
      desc: "Data-driven agricultural platform empowering farmers with AI crop insights.",
    },
    {
      title: "Nagesh PG",
      category: "LIVING",
      color: "from-red-400 to-pink-600",
      desc: "Modern co-living management software designed for seamless urban experiences.",
    },
    {
      title: "Interior Design",
      category: "ART",
      color: "from-rose-400 to-purple-600",
      desc: "Visualizing architectural beauty through cutting-edge 3D rendering technology.",
    },
    {
      title: "Veda Curate",
      category: "WEBDEV",
      color: "from-purple-400 to-blue-600",
      desc: "The project features modern UI design with integrated 3D models for an immersive user experience, and it is fully deployed online using a cloud hosting platform. The site serves as a professional service platform showcasing creative branding, design, and web development offerings.",
    },
  ];
    ...
    <div className="w-full h-screen bg-black overflow-hidden relative">
      ...
      <div className="fixed left-12 top-1/2 -translate-y-1/2 z-50 hidden lg:flex flex-col gap-8">
        {projectInfo.map((_, i) => (
          ...
          >
            <div
              className={`h-[2px] transition-all duration-500 ${i === activeSection ? "w-12 bg-primary" : "w-6 bg-white/10 group-hover:w-12 group-hover:bg-primary"}`}
            />
            <span
              className={`text-[10px] font-mono tracking-tighter transition-all duration-500 ${i === activeSection ? "text-white" : "text-white/20 group-hover:text-white"}`}
            >
              0{i + 1}
            </span>
          </div>
      ...
      <Canvas
        ...
        dpr={[1, 2]}
        onCreated={(state) =>
          (state.scene.background = new THREE.Color("#080808"))
        }
      ...
      >
        ...
        <Suspense
          fallback={
            <Html
              center
              className="text-white font-mono tracking-widest text-xs uppercase animate-pulse"
            >
              Syncing Assets...
            </Html>
          }
            ...
            <Scroll html>
              <div className="w-screen">
                {projectInfo.map((info, i) => (
                  <section
                    key={i}
                    className={`h-screen flex items-center px-10 md:px-40 ${i % 2 === 0 ? "justify-start" : "justify-end"}`}
                  >
                    <div className="max-w-2xl relative">
                      ...
                      <div className="relative z-10 space-y-8">
                        ...
                        <h1
                          className={`text-7xl md:text-[9rem] font-black leading-[0.8] tracking-tighter text-transparent bg-clip-text bg-gradient-to-br ${info.color} uppercase italic`}
                        >
                        ...
                        <div className="flex gap-8 pt-6">
                          <button
                            ...
                            className="group relative px-10 py-5 bg-white text-black font-black text-xs tracking-widest rounded-full overflow-hidden transition-all"
                          >
